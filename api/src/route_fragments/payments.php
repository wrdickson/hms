  ...
  if($v_result['valid']) {
    ...
    if( $payment_posted && $payment_id) {
      ...
      foreach($sale_items as $sale_item) {
        ...
        try{
          $stmt = $pdo->prepare("INSERT INTO sale_items ( folio, payment_id, sale_type, posted_by, description, sale_datetime, sale_quantity,
          sale_price, sale_subtotal, sale_tax, sale_total, tax_types, tax_spread)
          VALUES (:folio, :payment_id, :sale_type, :posted_by, :description, :sale_datetime, :sale_quantity, :sale_price, :sale_subtotal, :sale_tax, :sale_total, :tax_types, :tax_spread)");
          $stmt->bindParam(":folio", $res_folio);
    ...
    if( $payment_posted && $sale_items_posted ) {
      $pdo->commit();
    }
